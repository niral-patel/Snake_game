...
my_font = pg.font.SysFont('Times New Roman', 25, bold=True)
clock = pg.time.Clock()
...
def show_score(choice, color):
    score_surface = my_font.render('Score : ' + str(score)  + ' & Speed : ' + str(snake_speed), True, color)
    score_rect = score_surface.get_rect()
    if choice == 1:
        score_rect.midtop = (50, screen_height/5)
    else:
...
# Game Over
def gover():
    gover_surface = my_font.render('Game Over', True, red)
    gover_rect = gover_surface.get_rect()
    gover_rect.midtop = (screen_width/2, screen_height/4)
    screen.blit(gover_surface, gover_rect)
    text_again = "Press \'C' - Play Again and Press \'Q\' -Quit"
    again = my_font.render(text_again,True, red)
    again_rect = again.get_rect()
    again_rect.midtop = (screen_width/2, screen_height/2)
...
def main():
    ...
    while True:
        ...
        # Snake food
        pg.draw.rect(screen, blue, pg.Rect(food_pos[0], food_pos[1], 10, 10))
